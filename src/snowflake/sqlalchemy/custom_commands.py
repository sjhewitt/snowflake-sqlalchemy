...
class InsertAll(UpdateBase):
    __visit_name__ = "insert_all"
    _bind = None
    ...
    def into(self, table, columns=None, values=None):
        ...
        self.clauses.append((None, table, columns, values))
        return self
    ...
    def when(self, condition, table, columns=None, values=None):
        ...
        self.clauses.append((condition, table, columns, values))
        return self

    def else_(self, clause):
        if self.is_conditional:
            raise ValueError("Cannot set ELSE on a Conditional multi-table insert")
        self.else__ = clause
        return self
